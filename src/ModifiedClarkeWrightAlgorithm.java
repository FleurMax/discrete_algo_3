...
public class ModifiedClarkeWrightAlgorithm implements VRPAlgorithm{
    ...
    private ModifiedClarkeWrightAlgorithm(ModifiedClarkeWrightAlgorithm.Builder builder) {
    ...
    public static class Builder {
        ...
        public ModifiedClarkeWrightAlgorithm.Builder lambda(double lambda) {
            this.lambda = lambda;
            return this;
        }

        public ModifiedClarkeWrightAlgorithm.Builder mu(double mu) {
            this.mu = mu;
            return this;
        }

        public ModifiedClarkeWrightAlgorithm.Builder nu(double nu) {
            this.nu = nu;
            return this;
        }

        public ModifiedClarkeWrightAlgorithm build() {
            return new ModifiedClarkeWrightAlgorithm(this);
        }
    }
    ...
    public List<List<Customer>> solve(VRPProblem problem) {
        ...
        //Getting the average demand
        double aveDemand = 0;
        for (int i = 0; i < customers.size(); i++) {
            aveDemand += customers.get(i).getDemand();
        }
        aveDemand = aveDemand/customers.size();
        //Filling the pq
        for (int i = 0; i < customers.size(); i++) {
            ...
            for (int j = 0; j < i; j++) {
                double diffNorm = (depotDistance.get(i) > depotDistance.get(j)) ? depotDistance.get(i) - depotDistance.get(j) : depotDistance.get(j) - depotDistance.get(i);
                double savingsValue = depotDistance.get(i)+depotDistance.get(j)-lambda*customers.get(i).distanceTo(customers.get(j)) + mu*diffNorm - nu*(customers.get(i).getDemand() + customers.get(j).getDemand())/aveDemand;
                Savings s = new Savings(i,j,savingsValue);
                savingsQueue.add(s);
            }
        }
    ...
    public String getName() {
        return "Modified Clarke and Wright savings algorithm (Senne)";
    }
