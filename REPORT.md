...
### 3.1 Set A - Average Algorithm Comparison
This section summarizes the performance of all implemented algorithms across the Augerat Set A instances.

| Algorithm | Avg. Gap (%) | Avg. Time (ms) | Strategy |
| :--- | :---: | :---: | :--- |
| **Clarke-Wright (CW)** | 6.47% | ~3.0 | Greedy Savings |
...
The CW algorithm performs quite well on the Augerat Set A. The gap stays between 2.5% and 12.1%. On top of the competitive performance, the CW algorithm is very fast, often only a couple of miliseconds. To further illustrate its's performance we may compare it to the LNS in the following table.
...
| **Average** | — | — | — | **\-15.89** | **\-508.88** | **6.47%** | **6.84%** |
...
The algorithm scores below the Clarke-Wright vairants and Neural LNS, with gaps typically ranging between 10% and 50%. This is consistent with the findings of Gromicho et al. (2011), who report that RDP as a pure construction heuristic is less competitive on classical VRP variants, but performs significantly better when more realistic constraints are added. 

The computation times are reasonable: from more or less 45 ms for small instances up to 583 ms for the largest instance (n=100). This is slower than Clarke-Wright, but competitive with Neural LNS.  

### 3.4 Neural Large Neighborhood Search (Maxim)
Neural Large Neighborhood Search (Neural LNS) is a metaheuristic that enhances the traditional LNS framework by employing data-driven destroy and repair operators. This optimized version follows Hottung & Tierney (2020) with parallel variant exploration.

#### Optimization Improvements
- **Parallel Batch Processing**: Replaced the sequential variant loop with a parallel implementation, providing a ~3-4x speedup on multi-core systems.
- **Point-based Destroy & Tour-splitting**: These operators reduced the average gap from 6.84% to 2.55% by providing the repair operator with more flexible "incomplete tours."

| Instance | Optimal Cost | Neural LNS (Cost) | Time (ms) | Gap (%) |
| :--- | :---: | :---: | :---: | :---: |
| **A-n32-k5.vrp** | 784 | 787.08 | 2616.87 | 0.39% |
| **A-n33-k6.vrp** | 742 | 745.36 | 3483.02 | 0.45% |
| **A-n37-k5.vrp** | 669 | 691.00 | 3312.89 | 3.29% |
| **A-n39-k5.vrp** | 822 | 845.12 | 3374.20 | 2.81% |
| **A-n45-k7.vrp** | 1146 | 1155.07 | 3391.47 | 0.79% |
| **A-n53-k7.vrp** | 1010 | 1016.07 | 4251.53 | 0.60% |
| **A-n55-k9.vrp** | 1073 | 1086.52 | 4323.59 | 1.26% |
| **A-n64-k9.vrp** | 1401 | 1410.89 | 4810.98 | 0.71% |
| **A-n65-k9.vrp** | 1174 | 1215.88 | 5336.34 | 3.57% |
| **A-n69-k9.vrp** | 1159 | 1183.66 | 5000.62 | 2.13% |
| **A-n80-k10.vrp** | 1763 | 1828.55 | 5628.54 | 3.72% |
